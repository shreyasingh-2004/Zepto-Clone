...
const CartSidebar = ({isOpen,onClose}) => {
    ...
    const totalItems = cart.reduce((sum, items) => sum + items.quantity, 0);
          ...
          <div className="fixed right-0 top-0 h-full sm:w-96 w-full bg-white shadow-2xl z-50 transform transition-transform duration-300 ease-in-out flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {cart.length === 0 ? (
                ...
                cart.map((item) => (
                  <div key={item.id} className="flex items-center space-x-3 bg-gray-50 p-3 rounded-xl hover:shadow-sm transition-shadow">
                    <img 
                      src={item.image} 
                      alt={item.title}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[#00B207] font-semibold text-sm">
            ...
            <div className="p-4 border-t">
              ...
              <button className="w-full bg-[#00B207] text-white py-2 rounded-lg">Checkout</button>
            </div>
          </div>
        </>
    );
